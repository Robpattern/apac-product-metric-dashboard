...
export function classifyProject(listKey, name) {
  ...
  // QA automation. Checked after the AI/compliance rule so genuine product automation
  // still lands in that project — this is Jerry Lin's test tooling (API自动化, UI自动化).
  if (/自动化|测试自动化|\bqa\b|test automation|ui automation|api automation|automation script|regression suite/.test(n)) {
    return { project: EXCLUDED, confident: true };
  }
  // IT / ops support and scraping maintenance. Real work, but run by ops rather than the
  // product owners this board reports on, so it follows Data foundations out.
  if (/it ?& ?sea|it support|sea support|ops support|helpdesk|on-?call|standardisation|standardization|daily number of scraping/.test(n)) {
    return { project: EXCLUDED, confident: true };
  }
