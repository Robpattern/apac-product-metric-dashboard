...
export const METRICS = {
  ...
  t12:     { name: "T12 run rate",       theme: "Revenue", target: "50.8 → 14.8 months" },
...
export const PROJECT_BY_TICKET = {
  ...
  // On the Design Portfolio board but really AI automation work, per the confirmation
  // sheet, which files both under William's AI / compliance automation.
  "ATECH-7696": "aiauto", // Automation Service Intake Pipeline Design
  "ATECH-7762": "aiauto", // Automation Service Intake Portal Prototype

  // Ruyi tickets pulled out of exec reporting by hand. No rule covers these — they're
  // ordinary Ad Tech bugs that you decided not to report, not a category.
  ...
  "ATECH-6288": EXCLUDED, // ALMM 万相台无界 Scraping & Tmall IT sub-account ads permissions

  // Removed as redundant once the rules were fixed — the classifier now gets these right
  // on its own: 6836 (demand planning → Inventory China), 6781 / 6839 / 10965 / 7298
  // (Discovery Pipeline → Predict China), 7422 / 7822 / 2488 (Technical Roadmap, no
  // longer read at all).
};
...
// rollUp is off everywhere: every ticket counts as itself, which keeps the dashboard
// consistent with how the teams track work in ClickUp and in the planning sheet. Rolling
// Nimbus up to epics made one Predict module read as a single ticket and made the team
// look far smaller than it is. Flip a folder to true if that team ever needs collapsing.
export const FOLDERS = [
  { key: "nimbus",         id: "90174798289", team: "Team Nimbus",        rollUp: false },
...
// Individual lists, read directly rather than by enumerating a folder.
// Product Portfolio is deliberately narrowed to Discovery Pipeline: its other list,
// Technical Roadmap, is infrastructure and process work that was being excluded anyway
// (64 of the 69 dropped tickets came from there), so reading it only added noise.
export const STANDALONE_LISTS = [
  { key: "automation", id: "901715358790", team: "AI Automation",     listName: "Automation Planning" },
  { key: "discovery",  id: "901704266997", team: "Product Portfolio", listName: "Discovery Pipeline" },
];
...
export function classifyProject(listKey, name) {
  ...
  // Retro and process follow-ups. Checked first, because these titles mention whatever the
  // retro was about — "Coupang scraping", "Ads cross-check" — and would otherwise be
  // classified as product work in Predict Korea/Japan or Ad Tech. They're team process.
  if (/\[retro|retro\s*-|sprint\s*\d+\s*retro|knowledge sharing|retrospective/.test(n)) {
  ...
  // "demand planning" and "reorder" are inventory work — ATECH-6836 (Demand Driven
  // Automated Reorder Plan) had no other inventory keyword in its title.
  // 库存 = inventory, 补货 = replenishment. These were only in the China sub-test below, so a
  // Chinese-language inventory title never reached it — ATECH-7756 landed in Predict China.
  if (/inventory|sell-through|sell through|purchase order|\bpo\b|dio|replenish|stock|shipment|demand planning|reorder|库存|补货/.test(n)) {
  ...
  // Note: QA automation (API自动化, UI自动化) and IT / ops support were briefly excluded
  // here too. Reverted — only technical/infrastructure work is excluded, so that QA and
  // ops work stays visible. It lands in Predict China via the Nimbus fallback below,
  // which is a known imprecision rather than a statement about ownership.
  // Predict reporting and partner feedback. "BM Feedback" / "OP Feedback" are brand- and
  // ops-manager requests against the Predict reports, and the report names (daily, weekly,
  // monthly, download, trend chart) are Predict features. Without these, the titles carried
  // no matchable keyword and fell through to a guess.
  if (/bm feedback|op feedback|daily report|weekly report|monthly report|download report|trend chart|日报|周报|月报/.test(n)) {
    return hit("predictcn");
  }
  ...
  // Discovery Pipeline is overwhelmingly Predict China product discovery — it used to fall
  // back to Predict Korea/Japan, which is why so many Rey tickets needed a manual override.
  // Genuine KR/JP work is caught above by coupang/naver/rakuten/korea/japan keywords.
  const fallback = { ruyi: "adtech", automation: "aiauto", discovery: "predictcn", nimbus: "predictcn" };
